...
/**
 * @file
 * umd_global_base module file for utility functions.
 */
 ...
 */
function umd_global_base_get_node_type_id() {
 ...
 */
function umd_global_base_cmp($a, $b) {
 ...
 * Get block content.
 */
function umd_global_base_load_block($block_id, $get_rendered_result = FALSE) {
  if (empty($block_id)) {
    return FALSE;
  }
  $block_content = '';
 ...
 */
function umd_global_base_render_field($field, $bundle = 'node') {
 ...
 * Get term label.
 */
function umd_global_base_get_term_label($tid) {
 ...
 */
function umd_global_base_load_entities($conditions = [], $sorts = [], $range = FALSE, $get_builder = FALSE, $builder_opts = ['node', ['default']]) {
  $data = [];
 ...
 * Get menu items.
 */
function umd_global_base_get_menu_links($menu_name = 'main') {
  $menu = \Drupal::service('menu.tree_storage')->loadByProperties([
    'menu_name' => $menu_name,
    'enabled' => 1,
  ]) ?: [];

  usort($menu, "umd_global_base_cmp");
  return $menu;
}
 ...
 */
function umd_global_base_get_first_menu_url($menu_name = 'main') {
  // Redirect to first Bidder menu page.
  $newPath = '';
  $menuItems = umd_global_base_get_menu_links($menu_name);
